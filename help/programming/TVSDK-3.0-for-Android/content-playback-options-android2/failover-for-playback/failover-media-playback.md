...
uuid: 7eb4c157-b422-49ae-a753-4ad6b4e48b67
index: y
internal: n
snippet: y
---
...
When TVSDK cannot obtain an alternative segment, it triggers a `CONTENERROR` error notification. This notification contains an inner notification with the `DOWNLOAD_ERROR` code. If the stream with the problem is an alternate audio track, TVSDK generates the `AUDIO_TRACK_ERROR` error notification.
